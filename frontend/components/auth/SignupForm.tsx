...
export function SignupForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (error) setMsg(error.message);
      ...
      setMsg("Clearance verification link transmitted to inbox.");
  ...
  return (
    <form onSubmit={handleSignup} className="p-6 bg-slate-900 border border-slate-800 rounded-xl space-y-4">
      <h3 className="text-md font-bold text-white font-mono">REQUEST CLEARANCE</h3>
      ...
      <button type="submit" className="w-full py-2 bg-slate-800 text-white rounded text-xs">CREATE ACCOUNT</button>
    </form>
  );
}
