...
export default function TacticalHistory({ onReopen, languageCode, idToken }: TacticalHistoryProps) {
  ...
  useEffect(() => {
    ...
    if (idToken) {
      ...
      .then(data => {
        if (data.success) {
          const dbReports: HistoryReport[] = data.logs.map((log: any) => ({
            id: `db_${log.id}`,
            disasterId: log.disasterId,
            disasterName: log.disasterId.charAt(0).toUpperCase() + log.disasterId.slice(1),
            locationName: `Postgres Database Record (${parseFloat(log.lat).toFixed(4)}, ${parseFloat(log.lng).toFixed(4)})`,
            lat: log.lat,
            lng: log.lng,
      ...
      setReports(initialReports.filter(r => !r.id.startsWith("db_")));
    }
  }, [idToken]);
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {report.severity && (
                          <span className={`text-[8px] font-black font-mono px-1 py-0.5 rounded border leading-none transition uppercase ${
                            report.severity === "CRITICAL" || report.severity === "EXTREME" ? "bg-red-500/10 border-red-500/25 text-red-400" : 
                            report.severity === "HIGH" || report.severity === "SEVERE" ? "bg-orange-500/10 border-orange-500/25 text-orange-400" : 
                            report.severity === "MEDIUM" || report.severity === "MODERATE" ? "bg-amber-500/10 border-amber-500/25 text-amber-400" : 
                            "bg-emerald-500/10 border-emerald-500/25 text-emerald-400"
                          }`}>
                            {report.severity === "CRITICAL" || report.severity === "EXTREME" ? "🔴 CRITICAL" : 
                             report.severity === "HIGH" || report.severity === "SEVERE" ? "🟠 HIGH" : 
                             report.severity === "MEDIUM" || report.severity === "MODERATE" ? "🟡 MEDIUM" : 
                             "🟢 LOW"}
                          </span>
                        )}
                      </div>
